...
- **Symptom:** a correctly-deployed change to an *unhashed* fetched file (authoring batches, `data/texts/*.json`) never shows up in the browser, even after the hashed JS/CSS clearly updated. → **Cause (two-layer):** (1) `sw.js`'s cache-first strategy for non-navigation fetches has no expiry for URLs that never change name — once cached, served forever unless `VERSION` changes, and it was a hardcoded literal that never moved between deploys; (2) even fixing that, the fetch handler's `caches.match(req)` is **unscoped** — it searches every cache in storage by name-agnostic lookup, so a lingering old-version cache (a race on `activate`, or any partial state) shadows fresh content regardless of `VERSION`. → **Fix:** `vite.config.ts` now stamps `sw.js`'s `VERSION` with the build's git hash (`swVersionStamp`, same tail as the header build stamp) so every deploy gets a fresh cache automatically; `sw.js` reads only via `caches.open(VERSION).then(c => c.match(...))`, never the global `caches.match`. Verify by planting a stale response under an old `VERSION` cache key and confirming a fetch returns network content, not the stale entry — this is how the bug was found and how the fix was proven (astro-49x, 2026-07-10).
