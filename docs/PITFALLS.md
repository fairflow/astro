...
- **Symptom:** `bd` (beads) commands read/write the WRONG project's issues. → **Cause:** bd binds to the Finance workspace from any directory on this machine. → **Fix:** astro uses **GitHub issues** (`gh issue list`); never bd in this repo.
